...
import { ElectronBlocker, adsAndTrackingLists } from "@cliqz/adblocker-electron"; // Ad-Block relevant
import fetch from "cross-fetch"; // Ad-Block relevant
...
const enableAdBlocker = async (): Promise<ElectronBlocker> => {
  // ------------------------- AD-BLOCKER -------------------------
  ...
  //const blocker = ElectronBlocker.fromLists(fetch, fullLists, {
  //  enableOptimizations: true
  //})
  //
  // blocker.then(r => r.enableBlockingInSession(ytmView.webContents.session))
  //
  // blocker.then(r => r.on('request-blocked', request => {
  //   log.info('blocked', request.tabId, request.url)
  // }))
  //
  // blocker.then(r => r.on('request-redirected', request => {
  //   log.info('redirected', request.tabId, request.url)
  // }))
  //
  // blocker.then(r => r.on('request-whitelisted', request => {
  //   log.info('whitelisted', request.tabId, request.url)
  // }))
  //
  // blocker.then(r => r.on('csp-injected', request => {
  //   log.info('csp', request.url)
  // }))
  //
  // blocker.then(r => r.on('script-injected', (script, url) => {
  //   log.info('script', script.length, url)
  // }))
  //
  // blocker.then(r => r.on('style-injected', (style, url) => {
  //   log.info('style', style.length, url)
  // }))
